$(document).ready(function(){
	...
	//ing() is a funtion fetching ingredients record from database whenever page is load
	function ing(){
		$.ajax({
			url	:	"action.php",
			method:	"POST",
	...
	$("body").delegate(".ing","click",function(event){
		event.preventDefault();	
		$("#get_product").html("<h3>Loading...</h3>");	
		var bid = $(this).attr('bid');
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{get_selected_Ingredient:1,ing_id:bid},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
	...
	*/
	$("#search_btn").click(function(){
		...
		if(keyword != ""){
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{search:1,keyword:prod_keyword},
			success	:	function(data){ 
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
